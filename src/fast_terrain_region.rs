use godot::{
    classes::{image::Format, resource_saver::SaverFlags, Image, ResourceSaver},
    global::Error,
    prelude::*,
};

const COLOR_BLACK: Color = Color::from_rgb(0.0, 0.0, 0.0);
const COLOR_CONTROL: Color = Color::from_rgba(0.0, 0.0, 0.0, 0.0);
const COLOR_ROUGHNESS: Color = Color::from_rgb(1.0, 1.0, 1.0);
const COLOR_NAN: Color = Color::from_rgba(0.0, 0.0, 0.0, 0.0);
...
impl MapType {
    pub const FORMATS: [Format; 4] = [
        ...
        Format::RF,    // Control
        Format::RGBA8, // Color
        Format::MAX,   // Proper size of array
    ];
    ...
    const COLORS: [Color; 4] = [
        ...
        COLOR_ROUGHNESS, // Color
        COLOR_NAN,      // Max, unused
    ];
}
...
impl FastTerrainRegion {
    ...
    const TYPE_STRS: [&'static str; 3] = ["Height", "Control", "Color"];
    ...
    fn set_version(&mut self, version: f32) {
        ...
        if self.version < FastTerrainData::CURRENT_VERSION {
            godot_warn!(
                "Region {} version {:.3} will be updated to {:.3} upon save",
                self.base().get_path(),
                self.version,
                FastTerrainData::CURRENT_VERSION
            );
        }
    }
    ...
    fn set_maps(&mut self, maps: Array<Gd<Image>>) {
        if maps.len() != MapType::Max as usize {
            godot_error!(
                "Expected {} maps. Received {}",
                MapType::Max as usize - 1,
                maps.len()
            );
            return;
        }
        self.region_size = 0;
        self.set_height_map(maps.get(MapType::Height as usize));
        self.set_control_map(maps.get(MapType::Control as usize));
        self.set_color_map(maps.get(MapType::Color as usize));
    }

    fn get_maps(&self) -> Array<Gd<Image>> {
        godot_print!("Retrieving maps from region: {}", self.location);
        let mut maps = Array::new();
        if let Some(ref map) = self.height_map {
            maps.push(map);
        }
        if let Some(ref map) = self.control_map {
            maps.push(map);
        }
    ...
    fn set_control_map(&mut self, map: Option<Gd<Image>>) {
        godot_print!(
            "Setting control map for region: {}",
            if self.location.x != i32::MAX {
                self.location.to_string()
            } else {
                "(new)".into()
    ...
    fn set_color_map(&mut self, map: Option<Gd<Image>>) {
        ...
        // Sanitize and get new map
        let mut new_map = self.sanitize_map(MapType::Color, map);
        
        // Generate mipmaps if needed
        if let Some(color_map) = &mut new_map {
    ...
    fn sanitize_map(&self, map_type: MapType, map: Option<Gd<Image>>) -> Option<Gd<Image>> {
        ...
        if let Some(input_map) = map {
            if self.validate_map_size(&input_map) {
                if input_map.get_format() == format {
                    godot_print!(
                        "Map type {} correct format, size. Mipmaps: {}",
                        type_str,
                        input_map.has_mipmaps()
                    );
                    result = Some(input_map);
                } else {
                    godot_print!(
                        "Provided {} map wrong format: {:?}. Converting copy to: {:?}",
                        type_str,
                        input_map.get_format(),
                        format
                    );
                    ...
                    if new_map.get_format() == format {
                        ...
                        godot_print!(
                            "Cannot convert image to format: {:?}. Creating blank",
                            format
                        );
                    }
                }
                ...
                godot_print!(
                    "Provided {} map wrong size: {}. Creating blank",
                    type_str,
                    input_map.get_size()
                );
            }
        ...
        result.or_else(|| {
            godot_print!(
                "Making new image of type: {} and generating mipmaps: {}",
                type_str,
                map_type == MapType::Color
            );

            let new_map = Image::create_from_data(
                self.region_size,
                self.region_size,
                map_type == MapType::Color,
                format,
    ...
    fn validate_map_size(&self, map: &Gd<Image>) -> bool {
        ...
        if self.region_size != size.x || self.region_size != size.y {
            godot_error!(
                "Image size doesn't match existing images in this region: {}",
                size
            );
            return false;
        }
    ...
    fn calc_height_range(&mut self) {
        if let Some(height_map) = &self.height_map {
            ...
            if self.height_range != range {
                ...
                self.modified = true;
                godot_print!(
                    "Recalculated new height range: {} for region: {}. Marking modified",
                    range,
                    if self.location.x != i32::MAX {
                        self.location.to_string()
                    } else {
    ...
    fn save(&mut self, path: GString, sixteen_bit: bool) -> Error {
        ...
        if self.location.x == i32::MAX {
            godot_error!(
                "Region has not been setup. Location is INT32_MAX. Skipping {}",
                path
            );
            return Error::FAILED;
        }
        ...
        let result = if sixteen_bit {
            ...
            if let Some(height_map) = &self.height_map {
                ...
                let save_result = ResourceSaver::singleton()
                    .save_ex(&self.base().cast::<Resource>())
                    .path(&self.base().get_path())
    ...
    pub fn update_height(&mut self, height: f32) {
        if height < self.height_range.x {
            self.height_range.x = height;
            self.modified = true;
        } else if height > self.height_range.y {
            self.height_range.y = height;
            self.modified = true;
        }
    }

    pub fn update_heights(&mut self, low_high: Vector2) {
        if low_high.x < self.height_range.x {
            self.height_range.x = low_high.x;
            self.modified = true;
        }
        if low_high.y > self.height_range.y {
            self.height_range.y = low_high.y;
            self.modified = true;
        }
    }
}
...
impl IResource for FastTerrainRegion {
    fn init(base: Base<Resource>) -> Self {
        Self {
            base,
            version: 0.8, // Set to first version to ensure we always upgrade this
            region_size: 0,
            vertex_spacing: 1.0,
...
impl FastTerrainRegion {
    ...
    fn get_data(&self) -> Dictionary {
        let mut dict = Dictionary::new();        dict.insert("location", self.location);        dict.insert("deleted", self.deleted);
    ...
    fn set_data(&mut self, data: Dictionary) {
        if data.contains_key("location") {
            self.location = data.get("location").unwrap().to::<Vector2i>();
        }
        if data.contains_key("deleted") {
            self.deleted = data.get("deleted").unwrap().to::<bool>();
        }
        if data.contains_key("edited") {
            self.edited = data.get("edited").unwrap().to::<bool>();
        }
        if data.contains_key("modified") {
            self.modified = data.get("modified").unwrap().to::<bool>();
        }
        if data.contains_key("version") {
            self.version = data.get("version").unwrap().to::<f32>();
        }
        if data.contains_key("region_size") {
            self.region_size = data.get("region_size").unwrap().to::<i32>();
        }
        if data.contains_key("vertex_spacing") {
            self.vertex_spacing = data.get("vertex_spacing").unwrap().to::<f32>();
        }
        if data.contains_key("height_range") {
            self.height_range = data.get("height_range").unwrap().to::<Vector2>();
        }
        if data.contains_key("height_map") {
            self.height_map = data.get("height_map").unwrap().to::<Option<Gd<Image>>>();
        }
        if data.contains_key("control_map") {
            self.control_map = data.get("control_map").unwrap().to::<Option<Gd<Image>>>();
        }
        if data.contains_key("color_map") {
            self.color_map = data.get("color_map").unwrap().to::<Option<Gd<Image>>>();
        }
        if data.contains_key("instances") {
            self.instances = data.get("instances").unwrap().to::<Dictionary>();
        }
    }
}
